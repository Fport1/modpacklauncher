...
export default function InstanceDetailModal({ instance, onClose }: Props) {
  ...
  const isModpack = !!instance.modpackUrl
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              {isModpack ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center py-8">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-text-primary">Instancia gestionada por modpack</p>
                    <p className="text-xs text-text-muted mt-1 max-w-xs">Los archivos de esta instancia son gestionados por el modpack. Para modificar mods, desvincula la instancia.</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <button onClick={handleCheckUpdate} disabled={checkingUpdate}
                      className="flex items-center gap-1.5 px-4 py-2 border border-border text-text-secondary hover:text-text-primary hover:border-accent/50 rounded-lg text-sm transition-colors disabled:opacity-50">
                      {checkingUpdate ? (
                      ...
                      {checkingUpdate ? 'Comprobando...' : 'Comprobar actualización'}
                    </button>
                    <button onClick={() => setUnlinkConfirm(true)}
                      className="flex items-center gap-1.5 px-4 py-2 border border-border text-text-muted hover:text-red-400 hover:border-red-400/40 rounded-lg text-sm transition-colors">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
                      Desvincular del modpack
                    </button>
                  </div>
                  {updateStatus && (
                    <p className={`text-sm ${updateStatus.hasUpdate ? 'text-accent' : 'text-green-400'}`}>
                      {updateStatus.hasUpdate ? `¡Nueva versión disponible: v${updateStatus.version}!` : 'El modpack está actualizado ✓'}
                    </p>
                  )}
                </div>
              ) : (
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              {isModpack ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center py-8">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-text-primary">Instancia gestionada por modpack</p>
                    <p className="text-xs text-text-muted mt-1 max-w-xs">Los resource packs de esta instancia son gestionados por el modpack.</p>
                  </div>
                </div>
              ) : (
                <>
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              {isModpack ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center py-8">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-text-primary">Instancia gestionada por modpack</p>
                    <p className="text-xs text-text-muted mt-1 max-w-xs">Los shaderpacks de esta instancia son gestionados por el modpack.</p>
                  </div>
                </div>
              ) : (
                <>
